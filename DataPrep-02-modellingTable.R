 ...
 # batching the 03-participant_rollup by timeline date
 ...
 exec<-function(startDate,endDate)
 {
   ...
   days<-days[!days %in% c(as.Date("2015-06-03"),as.Date("2015-06-04"))]
   j<-1  # pointer in the days vector
   for (j in seq_along(days))
   {
     ...
     if (class(try(dbGetQuery(con, paste0("select 1 from lol.timelines_",nameDate," limit 1;"))))!='try-error')
     {
       ...
       i<-2    # start from 03-2-participant_rollup
       for (i in 2:14)    # make sure it iterates on a vector. from 03-2 to 03-12
       {
