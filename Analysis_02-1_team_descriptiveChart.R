...
# discretise the given column by equal volume bin and calculate the avg, cumulative percentage and winrate
histPercent<-function(data,column,numInterval)
    {
        ...
        if (class(col)=='integer' | class(col)=='numeric')
            {
                a<-data$win
                DF<-merge(col,a,by="row.names") # Notice that x = given column, y = win 
...
# chart a number of end game measures against winrate
endGameChart<-function(data,measures,numInterval,tableMeasures)
    {
        ...
        #for (i in 1:1)
            {
                ...
                g<-ggplot() +
                        geom_line(data = ret,aes(x = avg, y = winrate),group = 1,size = 1) +
                        ...
                        xlab(measures[i])
                        #+ geom_point(aes(x = ret[rowNum,2],y = ret[rowNum,4]),size=5,colour="red")
        ...
        #                key<-colnames(ret)
        #                value<-unname(alply(ret,1,identity))
        #                modified<-list(key,value)
                        # m<-apply(a,1,jsonlite::toJSON)  # this makes numeric to string
...
# 
#unsensible:
#champLevel, goldEarned_perMin,largestCriticalStrike_perMin,longestTimeSpentLiving,
